# PhonePe Pulse Data Analysis — Streamlit Dashboard
# This app provides interactive visualizations for PhonePe transaction,
# insurance, and user engagement data stored in PostgreSQL.
...
# Database connection via environment variable (password not hardcoded)
...
###########
        ...
        with col1:
            fig_count = px.pie(
                ...
                title="Distribution of Total Transaction Count (Donut Chart)",
                hole=0.55
            )
        ...
        with col2:
            fig_amount = px.pie(
                ...
                names="Transaction_type",
                values="total_amount",
                title="Distribution of Total Transaction Amount (Donut Chart)",
                hole=0.55
            )
        ...
        #  TOP 10 STATES 
        st.markdown(
            "<h2 style='color:#e74c3c; text-align:center;'>Top 10 State-wise Total Transaction Amount (Bar Plot)</h2>",
            unsafe_allow_html=True
        )
        ...
        #  STATE-WISE CATEGORY
        st.markdown(
            "<h2 style='color:#e74c3c; text-align:center;'>Transactions by State and Payment Category (Line Plot)</h2>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<h2 style='color:#e74c3c; text-align:center;'>Transaction Mix (%) {RELATIVE STACKED BAR}</h2>",
            unsafe_allow_html=True
        )
        ...
        # TREND ANALYSIS 
        st.markdown(
            "<h2 style='color:#e74c3c; text-align:center;'>Trend Analysis (Bar Chart)</h2>",
            unsafe_allow_html=True
        )
    ...
    # 2020 Q1 doesn’t show top 10 states because insurance data for that quarter is incomplete. 
    # Many states don’t have records at all for that period, so when I filter by 2020 Q1, SQL only returns the states that actually exist in the data.
    # Since missing states are not treated as zero but as absent, the result naturally has fewer than 10 states.
    # This reflects real data availability, not a visualization error.
    ...
    # Second Scenario: Insurance Penetration and Growth Potential Analysis
        ...
        st.markdown(
            "<h3 style='color:#2980b9;'>Top 10 States by Insurance Transaction Amount (Simple Bar Chart.)</h3>",
            unsafe_allow_html=True
        )
...
##
        if df_top_states.empty:
            st.warning("No insurance data available for this Year and Quarter.")
            st.info(
                "📌 **Why is this empty?** PhonePe's insurance services were launched in "
                "Q2 2020. Therefore, Q1 2020 has no insurance transaction records in the dataset."
            )
...
##
    ...
    ################
        ...
        st.markdown(
            "<h3 style='color:#2980b9;'>Quarter-wise Insurance Trend (Simple Bar Chart.)</h3>",
            unsafe_allow_html=True
        )
...
###################
        ...
        st.markdown(
            "<h3 style='color:#2980b9;'>Insurance Growth Trend (Year-wise) (Line-Plot)</h3>",
            unsafe_allow_html=True
        )
...
#######################
        ...
        st.markdown(
            "<h3 style='color:#2980b9;'>State-wise Insurance Analysis (Line Plot)</h3>",
            unsafe_allow_html=True
        )
    ...
    # Third Scenario: User Engagement & Growth
        ...
        #  BAR PLOT (STATE ENGAGEMENT FOR SELECTED YEAR/Q)
        ...
        fig_eng_bar = px.bar(
            ...
            title=f"Top 10 States by Engagement ({eng_year_selected} Q{eng_quarter_selected}) (Bar Plot)",
            text_auto=".2s",
            color_discrete_sequence=["#0d6efd"]
        )
        ...
        #  LINE PLOT (COVID-STYLE TIME SERIES)
        ...
        fig_eng_line = px.line(
            ...
            y="total_transactions",
            markers=True,
            title="Total Transaction Count Over Years (Line Plot)"
        )
        ...
        #  STACKED AREA PLOT (ENGAGEMENT GROWTH)
    ...
    #######################
        ...
        fig_states = px.bar(
            ...
            y="total_transactions",
            text_auto=".2s",
            title=f"Top 10 States – Insurance Engagement ({year_selected} Q{quarter_selected}) (Bar Chart)",
    ...
    #######################
        ...
        fig_year = px.line(
            ...
            y="total_transactions",
            markers=True,
            title="Insurance Engagement Growth Over Years (Line Plot)"
        )
    ...
    #######################
        ...
        fig_state = px.line(
            ...
            y="total_transactions",
            markers=True,
            title=f"Insurance Engagement in {selected_state} – {year_selected} (Line Plot)"
        )
    ...
    # Fifth Scenario: User Registration Analysis
    ...
    #############
        ...
        fig_reg_states = px.bar(
            ...
            text_auto=".2s",
            title=f"Top 10 States – User Registrations ({reg_year_selected} Q{reg_quarter_selected}) (Bar Chart)",
    ...
    ########
        ...
        fig_reg_year = px.line(
            ...
            y="total_registered_users",
            markers=True,
            title="Registered Users Growth Over Years (Line Plot)"
        )
    ...
    ################
        ...
        fig_reg_state = px.line(
            ...
            y="total_registered_users",
            markers=True,
            title=f"User Registration Trend in {reg_state_selected} – {reg_year_selected} (Line Plot)"
        )
